...
# Removed NA rows (genes DEseq2 filtered out due to low expression )
res_tbl <- res_tbl[!is.na(res_tbl$padj), ]  

# Define significance
res_tbl$sig <- ifelse(res_tbl$padj < 0.05 & abs(res_tbl$log2FoldChange) >= 1,
                      "Significant (FDR < 0.05, |log2FC| ≥ 1)", 
                      "Not Significant")

# Create MA plot
ggplot(res_tbl, aes(x = log10(baseMean), y = log2FoldChange, color = sig)) +
  geom_point(alpha = 0.5, size = 1) +
  scale_color_manual(values = c("Significant (FDR < 0.05, |log2FC| ≥ 1)" = "red", 
...
**Figure 10. MA plot of differential gene expression.** Each point represents a gene. The x-axis shows mean expression level (log10 scale), and the y-axis shows log2 fold change between hypoxia and normoxia. Red points indicate significantly differentially expressed genes (FDR < 0.05, |log2FC| ≥ 1). Dashed lines mark the fold-change thresholds (±1).
